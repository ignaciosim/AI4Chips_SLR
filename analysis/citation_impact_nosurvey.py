"""Citation impact analysis for the AI4Chips corpus — SURVEYS EXCLUDED.

Excludes survey/review papers that skew citation metrics. Survey detection
is title-based (keywords: survey, review, overview, tutorial, taxonomy).

Otherwise identical to citation_impact.py. Uses JSON for citation counts,
CSV for method tags and chip tasks.

Sections:
  1. Overall citation statistics (with survey comparison)
...
def main():
    ...
    # Separate surveys from research papers
    surveys_excluded = []
    all_cites = []
    ...
    for p in papers:
        ...
        if is_survey(title):
            surveys_excluded.append({
                "year": year, "title": title, "cites": cites,
            })
            continue
    ...
    # =====================================================================
    print("=" * 80)
    print(f"SURVEYS EXCLUDED ({len(surveys_excluded)} papers removed)")
    print("=" * 80)
    survey_cites_total = sum(s["cites"] for s in surveys_excluded)
    for s in sorted(surveys_excluded, key=lambda x: -x["cites"]):
        print(f"  [{s['year']}] {s['cites']:>4} cites  {s['title'][:75]}")
    print(f"\n  Total survey citations removed: {survey_cites_total:,}")
    print(f"  Remaining corpus: {total_papers} research papers")
    ...
    print()
    print("=" * 80)
    print(f"CITATION IMPACT ANALYSIS — NO SURVEYS  (N = {total_papers} papers)")
    print("=" * 80)
    ...
    print()
    print("=" * 100)
    print("TOP 20 MOST-CITED RESEARCH PAPERS (surveys excluded)")
    print("=" * 100)
    ...
    # =====================================================================
    print("=" * 80)
    print("CITATION IMPACT BY AI METHOD (no surveys)")
    print("=" * 80)
    ...
    print()
    print("=" * 80)
    print("CITATION IMPACT BY CHIP TASK (no surveys)")
    print("=" * 80)
    ...
    print()
    print("=" * 80)
    print("CITATION IMPACT BY VENUE (no surveys)")
    print("=" * 80)
    ...
    print()
    print("=" * 100)
    print("AGE-NORMALIZED: TOP 20 BY CITATIONS PER YEAR (min 2 cites, no surveys)")
    print("=" * 100)
    ...
    print()
    print("=" * 80)
    print("CITATION BRACKETS: Papers by citation range per year (no surveys)")
    print("=" * 80)
    brackets = [
        ("0", lambda c: c == 0),
        ("1-5", lambda c: 1 <= c <= 5),
